...
function activateItem() {
  ...
  date.value = this.cells[4].innerText;
...
const updateItem = () => {
  ...
  if (tableRowIndex || tableRowIndex === 0) {
    if (name === "") {
      alert("item name can not be left empty");
      document.querySelector("#name").focus();
      return;
    ...
    if (confirm === true) {
      let store = storage.splice(tableRowIndex, 1, {
        name,
        quantity,
        price,
        totalPrice,
...
const deleteItem = () => {
  ...
  if (confirm === true) {
    const storage = JSON.parse(localStorage.getItem("productList"));
    let tableRowIndex = rowIndex - 1;
    console.log(`tableRowInde`, tableRowIndex);
